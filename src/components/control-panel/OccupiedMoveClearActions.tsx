  ...
  if (showMoveSelect) {
    return (
      <div className="space-y-1.5">
          ...
          <div className="flex flex-wrap gap-1">
            {availableForMove.map((t) => (
              <button
                key={t.id}
                onClick={() => onMove(t.id)}
                className="text-[10px] font-medium px-1.5 py-0.5 rounded-md bg-cream hover:bg-primary/10 text-charcoal border border-border hover:border-primary/30 transition-colors"
              >
                {t.label} · {t.seats}
              </button>
            ))}
        ...
        <button onClick={onCancelMoveSelect} className="text-[10px] text-charcoal-lighter hover:text-charcoal">취소</button>
      </div>
    )
  }
